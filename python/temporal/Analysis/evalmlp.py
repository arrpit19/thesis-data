...
def evalmlp():
    ...
    # Architecture
    d = pd.read_csv("C:/Users/Arpit/f.csv")
    ...
    pd.DataFrame({"f1": v1, "f2": v2, "f3":v3 }).to_csv('mlp_vloss.csv')
    pd.DataFrame({"f1": t1, "f2": t2, "f3":t3 }).to_csv('mlp_trainloss_.csv')
...
test_y = pd.read_csv("C:/Users/Arpit/Documents/ProfoundData/gpptest2008.csv" )
predmlp = pd.read_csv("C:/Users/Arpit/mlp_eval_preds_test.csv")
plt.plot(range(len(test_y)), test_y['GPP'], label='Observed GPP', marker='o')
plt.plot(range(len(predmlp)), predmlp['test_mlp1'], label='Predicted GPP by MLP', marker='o')

# Customize the plot
plt.title("GPP plot 2008 MLP", fontsize=16)
