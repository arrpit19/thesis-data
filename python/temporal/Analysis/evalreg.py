...
def evalreg():
    ...
    pd.DataFrame({"f1": t1, "f2": t2, "f3":t3}).to_csv('reg_trainloss.csv')
    v1 = []
    v2 = []
    v3 = []
    ...
    pd.DataFrame({"f1": v1, "f2": v2, "f3":v3}).to_csv('reg_vloss_.csv')
...
test_y = pd.read_csv("C:/Users/Arpit/Documents/ProfoundData/gpptest2008.csv" )
regmlp = pd.read_csv("C:/Users/Arpit/reg_eval_preds_test.csv")
plt.plot(range(len(test_y)), test_y['GPP'], label='Observed GPP', marker='o')
plt.plot(range(len(regmlp)), regmlp['test_reg1'], label='Predicted GPP by REG', marker='o')
plt.title("GPP plot 2008 REG", fontsize=16)
